...
func (d *Deps) Coverage(ctx context.Context, j *jobs.Job) error {
	...
	return d.DB.Tx(ctx, func(tx pgx.Tx) error {
		// Coverage and rules form one run-scoped materialized analysis view.
		// Serialize their snapshots so a slower rules pass cannot overwrite a
		// newer coverage result with stale findings.
		...
		if err := pool.QueryRow(ctx, `select count(*) from model_attempts where recording_id=$1 and source not like 'hook:%' and api_mode='unknown' and terminal_state <> 'error'`, rec).Scan(&c.UnparsedConnections); err != nil {
			return err
		}
		...
		_ = pool.QueryRow(ctx, `select relation_revision, transport_proof from capture_runs where id=$1`, run).Scan(&c.RelationRevision, &proofRaw)
		...
		if err := pool.QueryRow(ctx, `update recordings set coverage=$2, coverage_revision=coverage_revision+1, updated_at=now() where id=$1 returning coverage_revision`, rec, cb).Scan(&coverageRevision); err != nil {
			return err
		}
		// Resolve also schedules an early rules pass for fast relation findings.
		// Every committed coverage revision schedules a final refresh so coverage-
		// derived findings cannot remain stale.
		if _, err := jobs.Enqueue(ctx, tx, jobs.Spec{
			ProjectID:        j.ProjectID,
			Type:             jobs.TypeRules,
			CaptureRunID:     run,
			InputRef:         map[string]any{"coverage_recording_id": rec, "coverage_revision": coverageRevision, "relation_revision": c.RelationRevision},
			ProcessorVersion: RulesVersion,
			Priority:         2,
		}); err != nil {
			return err
		}
		return d.publishTx(ctx, tx, j.ProjectID, "recording", rec, "coverage", coverageRevision)
	})
}
