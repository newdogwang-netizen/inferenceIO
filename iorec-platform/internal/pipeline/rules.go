...
func (d *Deps) Rules(ctx context.Context, j *jobs.Job) error {
	...
	rows.Close()
	// attempt-level
	rows, err = pool.Query(ctx, `select id, recording_id, terminal_state, coalesce(first_seq,0), coalesce(last_seq,0), normalized->>'response_text', normalized->'messages', started_at, ended_at, model from model_attempts where capture_run_id=$1`, run)
	if err != nil {
		return err
	}
	type attemptLite struct {
		id, rec, terminal, model string
		first, last              int64
	}
	...
	for rows.Next() {
		...
		var model *string
		if err := rows.Scan(&a.id, &a.rec, &a.terminal, &a.first, &a.last, &respText, &msgs, &started, &ended, &model); err != nil {
			rows.Close()
			return err
		}
		if model != nil {
	...
	for _, rc := range recs {
		if rc.cov.TransportAttempts > 0 && float64(rc.cov.BodyUnavailable)/float64(rc.cov.TransportAttempts) > 0.10 {
			findings = append(findings, Finding{RuleID: "unattributed_ratio", Severity: "warn", Title: "超过 10% 的 attempt 无法归属（正文不可用）", Detail: map[string]any{"body_unavailable": rc.cov.BodyUnavailable, "attempts": rc.cov.TransportAttempts}, Evidence: []EvidenceRef{{rc.id, 1, 1}}, EvidenceKey: rc.id + "#" + fmt.Sprint(rc.cov.RelationRevision), RecordingID: rc.id})
		}
