...
func (s *Service) Run(ctx context.Context, j *jobs.Job) (retErr error) {
	...
	rows, err := s.DB.Pool.Query(ctx, `select jsonb_build_object(
		...
		'attempts',coalesce((select jsonb_agg(jsonb_build_object('id',a.id,'native_id',a.native_id,'entity_kind',a.entity_kind,'parent_attempt_id',a.parent_attempt_id,'projection',a.projection,'evidence_refs',a.evidence_refs,'terminal_state',a.terminal_state,'status_code',a.status_code,'provider_host',a.provider_host,'started_at',a.started_at,'ended_at',a.ended_at,'sse_event_count',a.sse_event_count) order by a.started_at,a.id) from model_attempts a where a.inference_id=i.id and a.project_id=i.project_id),'[]'::jsonb),
