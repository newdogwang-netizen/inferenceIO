...
- The endpoint proxy is the verified client transport path for HTTP/1.1, SSE, and reassembled WebSocket messages. A controlled 100-stream h2c test and a real local TLS/ALPN audit verify that concurrent HTTP/2 bodies do not cross between attempts. `transport-audit` independently reconstructs wire stream IDs and exact DATA bodies, but the endpoint proxy still does not expose upstream connection/stream IDs for one-to-one event correlation.
